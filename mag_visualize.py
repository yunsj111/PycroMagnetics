...
def show_MagProperties(mag_property, vmin=None, vmax=None,figsize=(5,5), fontsize=100):
    ...
    for i in range(total_nz):
        plt.subplot(d1,5,i+1)
        sns.heatmap(mag_property[i].tolist(), cmap='RdBu', vmin=vmin, vmax=vmax, cbar=True, yticklabels=False, xticklabels=False).invert_yaxis()
        plt.title('nz : {}/{}'.format(i,total_nz), fontdict={'size': fontsize})
    plt.show()